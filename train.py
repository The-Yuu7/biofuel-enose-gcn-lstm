...
from model.gcn_lstm import build_gcn_lstm_model, StaticGCNConv

def main():
    print("Fase 4: Entrenamiento y Evaluación del Modelo GCN-LSTM\n")
    ...
    # Determinar el número de clases dinámicamente
    num_classes = len(le.classes_)
    print(f"Número de clases detectadas: {num_classes} -> {list(le.classes_)}\n")
    
    # Exportar perfil de referencia de gasolina para diagnósticos
    try:
        ref_class_idx = list(le.classes_).index('referencia')
        ...
        if len(ref_windows) > 0:
            ...
            print(f"Perfil de referencia de gasolina guardado en '{perfil_ref_path}'")
        else:
            print("[WARN] No se encontraron ventanas para la clase 'referencia' en el conjunto de entrenamiento.")
    except ValueError:
        print("[WARN] No se encontró la clase 'referencia' en las etiquetas codificadas. No se pudo exportar el perfil de referencia.")
    
    # 2. Construir la arquitectura del modelo
    print("Construyendo el modelo GCN-LSTM...")
    model = build_gcn_lstm_model(input_shape=(30, 8), num_classes=num_classes)
    ...
    print(f"Pesos completos guardados exitosamente en '{model_keras_path}'")
    
    # 8. Exportar a formato TensorFlow Lite
    print("\nExportando modelo a TensorFlow Lite...")
    converter = tf.lite.TFLiteConverter.from_keras_model(model)
    # Ya no requerimos SELECT_TF_OPS porque unrolleamos el LSTM
    tflite_model = converter.convert()
    model_tflite_path = "model/enose_modelo.tflite"
    with open(model_tflite_path, "wb") as f:
        f.write(tflite_model)
    print(f"Modelo TFLite guardado exitosamente en '{model_tflite_path}'")
